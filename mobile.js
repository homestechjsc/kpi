...
window.switchTab = (tab) => {
    ...
    if (tab === 'today') {
        ...
        if (currentUser) {
            ...
            // 👉 Đổ dữ liệu Telegram ID hiện tại vào đây
            const currentId = currentUser.telegramId || '';
            const displayEl = document.getElementById('currentTelegramIdDisplay');
            const inputEl = document.getElementById('accTelegramId');
            if (displayEl) displayEl.textContent = currentId || 'Chưa cập nhật';
...
function renderAssignedTasks() {
    ...
    // Lọc công việc của user (Phụ trách hoặc Hỗ trợ)
    const myTasks = Object.entries(allAssignedTasks).filter(([id, task]) => {
        return task.ktPhuTrach === currentUser.name || task.ktHoTro === currentUser.name;
    }).reverse();

    // Đếm số việc chưa hoàn thành để hiển thị badge
    ...
    if (myTasks.length === 0) {
        container.innerHTML = '<p class="text-xs text-slate-400 text-center py-8 bg-slate-50 rounded-2xl border">Bạn chưa được phân công công việc nào.</p>';
        return;
    }
...
window.switchTab = (tab) => {
    ...
    // Ẩn tất cả các nội dung tab trước
    const todayContent = document.getElementById('todayTabContent');
    const reportContent = document.getElementById('reportTabContent');
    const accountContent = document.getElementById('accountTabContent');
    ...
    // Xử lý hiển thị theo từng tab được chọn
    if (tab === 'today') {
        ...
        if (currentUser) {
            ...
            // 👉 Cập nhật hiển thị Telegram ID cá nhân ở đây
            const currentId = currentUser.telegramId || '';
            const displayEl = document.getElementById('currentTelegramIdDisplay');
            const inputEl = document.getElementById('accTelegramId');
            if (displayEl) displayEl.textContent = currentId || 'Chưa cập nhật';
